...
skf = StratifiedKFold(n_splits=config.NUMBER_FOLDS, shuffle=True, random_state=42)
val_score = []
history = []
pred = []
for fold, (train_ind, valid_ind) in enumerate(skf.split(x_train, y_train)):
    print("fold", fold + 1)
    tf.tpu.experimental.initialize_tpu_system(tpu)
    train_data = create_dataset(x_train[train_ind], y_train[train_ind], val=False, batch_size=BATCH_SIZE, AUTO= AUTO)
    valid_data = create_dataset(x_train[valid_ind], y_train[valid_ind], val=True, batch_size=BATCH_SIZE, AUTO= AUTO)

    Checkpoint = tf.keras.callbacks.ModelCheckpoint(
        f"{config.MODEL}-{fold}.h5",
        monitor="val_loss",
        verbose=0,
        save_best_only=True,
    ...
    print(
        "fold {} validation accuracy {}".format(
            fold + 1, np.mean(train_history.history["val_accuracy"])
        )
    )
    print(
        "fold {} validation loss {}".format(
            fold + 1, np.mean(train_history.history["val_loss"])
        )
    )
